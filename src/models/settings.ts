...
export default class Settings {
  ...
  constructor(
    ...
    const borderStyle = this.activeScopeLineCSSElements.filter(
      (e) => e[0] === 'borderStyle'
    );
    ...
    this.highlightActiveScope = configuration.get(
    ...
    this.showVerticalScopeLine = configuration.get(
    ...
    this.showHorizontalScopeLine = configuration.get(
    ...
    if (this.colorMode === ColorMode.Consecutive) {
      const consecutiveSettings = configuration.get<[{}]>(
      ...
      if (consecutiveSettings.length < 3) {
        throw new Error(
          'consecutivePairColors expected at least 3 parameters, actual: ' +
            consecutiveSettings.length
        );
      }

      const orphanColor = consecutiveSettings[
      ...
      const colors = consecutiveSettings[
      ...
      consecutiveSettings
        ...
        .forEach((brackets, index) => {
          if (typeof brackets === 'string' || Array.isArray(brackets)) {
            ...
            this.bracketPairs.push(
              new BracketPair(brackets[0], brackets[1], colors, orphanColor)
            );
            return;
          }
      ...
      const independentSettings = configuration.get<[[{}]]>(
        'independentPairColors'
      );
      ...
      independentSettings.forEach((innerArray: unknown, index) => {
        if (!Array.isArray(innerArray)) {
          throw new Error(
            'independentPairColors[' + index + '] is not an array'
          );
        }
        ...
        if (brackets.length < 2) {
          throw new Error(
            'independentSettings[' + index + '][0] needs at least 2 elements'
          );
        }
        ...
        if (!Array.isArray(colors)) {
          throw new Error(
            'independentSettings[' + index + '][1] is not string[]'
          );
        }
        ...
        if (typeof orphanColor !== 'string') {
          throw new Error(
            'independentSettings[' + index + '][2] is not a string'
          );
        }
  ...
  public createGutterBracketDecorations(color: string, bracket: string) {
    ...
    const decoration =
      vscode.window.createTextEditorDecorationType(decorationSettings);
    return decoration;
  }
  ...
  private createBracketDecorations(

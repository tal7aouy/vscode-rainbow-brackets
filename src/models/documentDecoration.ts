...
export default class DocumentDecoration {
  ...
  private readonly stringStrategies = new Map<
    string,
    (
      content: string,
      lineIndex: number,
  ...
  >();
  private readonly stringOrTokenArrayStrategies = new Map<
    string,
    (
  ...
  constructor(document: vscode.TextDocument, settings: Settings) {
    ...
    this.largeFileRange = new vscode.Range(
      new vscode.Position(0, 0),
      new vscode.Position(5000, 0)
    );
  ...
  public expandBracketSelection(editor: vscode.TextEditor) {
    ...
    editor.selections.forEach((selection) => {
      ...
      const nextPos = this.document.validatePosition(
        selection.active.translate(0, 1)
      );
      const nextScope = this.getScope(nextPos);
      if (nextScope) {
        const start = this.document.validatePosition(
          nextScope.open.range.start.translate(0, 1)
        );
        const end = this.document.validatePosition(
          nextScope.close.range.end.translate(0, -1)
        );
    ...
    if (index < this.lines.length) {
      ...
      for (let i = this.lines.length; i <= index; i++) {
        ...
        const newLine = new TextLine(
          ...
          this.settings,
          i,
          previousLine.copyMultilineContext()
        );
  ...
  public triggerUpdateScopeDecorations(
  ...
  private updateScopeDecorations() {
    ...
    for (const scope of scopes) {
      ...
      if (this.settings.showBracketsInGutter) {
        if (scope.open.range.start.line === scope.close.range.start.line) {
          const decoration = this.settings.createGutterBracketDecorations(
            scope.color,
            scope.open.character + scope.close.character
          );
          event.textEditor.setDecorations(decoration, [
            scope.open.range,
            scope.close.range,
      ...
      if (this.settings.showBracketsInRuler) {
        const decoration = this.settings.createRulerBracketDecorations(
          scope.color
        );
        event.textEditor.setDecorations(decoration, [
          scope.open.range,
      ...
      const lastWhiteSpaceCharacterIndex = this.document.lineAt(
        scope.close.range.start
      ...
      const position = this.settings.scopeLineRelativePosition
        ? Math.min(
            scope.close.range.start.character,
            scope.open.range.start.character
          )
        : 0;
      ...
      for (let lineIndex = start - 1; lineIndex <= end; lineIndex++) {
        ...
        if (!line.isEmptyOrWhitespace) {
          ...
          leftBorderColumn = Math.min(
            leftBorderColumn,
            this.calculateColumnFromCharIndex(
              line.text,
              firstCharIndex,
              tabSize
            )
      ...
      if (this.settings.showVerticalScopeLine) {
        const verticalLineRanges: Array<{
          range: vscode.Range
          valid: boolean
        }> = [];
        ...
        for (let lineIndex = start; lineIndex <= endOffset; lineIndex++) {
          ...
          const linePosition = new vscode.Position(
            lineIndex,
            this.calculateCharIndexFromColumn(
              line.text,
              leftBorderColumn,
              tabSize
            )
        ...
        const safeFallbackPosition = new vscode.Position(
          start - 1,
          leftBorderIndex
        );
        this.setVerticalLineDecoration(
          scope,
          event,
      ...
      if (this.settings.showHorizontalScopeLine) {
        ...
        if (scope.open.range.start.line === scope.close.range.start.line) {
          underlineLineRanges.push(
            new vscode.Range(scope.open.range.start, scope.close.range.end)
          );
        } else {
          ...
          underlineLineRanges.push(
            new vscode.Range(leftStartPos, scope.open.range.end)
          );
          if (lastBracketIsFirstCharacterOnLine) {
            overlineLineRanges.push(
              new vscode.Range(leftEndPos, scope.close.range.end)
            );
          } else {
            underlineLineRanges.push(
              new vscode.Range(leftEndPos, scope.close.range.end)
  ...
  private setOverLineDecoration(
    ...
    const lineDecoration = this.settings.createScopeLineDecorations(
      scope.color,
      true,
      false,
      false,
  ...
  private setVerticalLineDecoration(
    ...
    const normalDecoration = this.settings.createScopeLineDecorations(
      scope.color,
      false,
      false,
      false,
  ...
  private updateLowestLineNumber(
    contentChanges: readonly vscode.TextDocumentContentChangeEvent[]
  ) {
    for (const contentChange of contentChanges) {
      this.lineToUpdateWhenTimeoutEnds = Math.min(
  ...
  private updateDecorations() {
    ...
    if (editors.length === 0) {
      console.warn(
        'No editors associated with document: ' + this.document.fileName
      );
      return;
    }
  ...
  private matchString(
    ...
    while ((match = this.settings.regexNonExact.exec(content)) !== null) {
      ...
      positions.push(
        new FoundBracket(new vscode.Range(startPos, endPos), match[0])
      );
    }
  ...
  private matchStringOrTokenArray(
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      if (indexes.has(i) && typeof content === 'string') {
        ...
        result = this.parseTokenOrStringArray(
          [content],
          lineIndex,
          charIndex,
          positions
